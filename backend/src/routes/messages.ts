...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const pageNum =
        typeof page === 'string' ? parseInt(page) : (page as number);
      const limitNum =
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    `;
      ...
      const [result, countResult] = await Promise.all([
        ...
        pool.query(countQuery, [currentUserId, otherUserId, listingId]),
      ]);
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const { receiver_id, message_text, listing_id }: SendMessageData =
        req.body;
...
router.put(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const result = await pool.query(
        'UPDATE messages SET read_status = true WHERE id = $1 AND receiver_id = $2 AND read_status = false RETURNING *',
        [messageId, userId]
      );
